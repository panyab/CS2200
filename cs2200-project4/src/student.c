    ...
    if (scheduler_algorithm == PR) {
        ...
        for(unsigned int i = 0; i < cpu_count; i++) {
            if (!current[i]) {
                pthread_mutex_unlock(&current_mutex);
                return;
            } else if (current[i]->priority > process->priority) {
                higher = 1;
            } else if (current[i]->priority > priority) {
                cpu_id = i;
                priority = current[i]->priority;
            }
        }
        pthread_mutex_unlock( & current_mutex);
        if (higher == 1) {
