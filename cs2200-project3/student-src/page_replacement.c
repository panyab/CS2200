...
pfn_t free_frame(void) {
    ...
    if (frame_table[victim_pfn].mapped) {
        pfn_t victim_ptbr = frame_table[victim_pfn].process->saved_ptbr;
        pte_t* victim_pfe = (pte_t*) (mem + (victim_ptbr * PAGE_SIZE)) + frame_table[victim_pfn].vpn;
        if (victim_pfe->dirty) {
            stats.writebacks++;
            swap_write(victim_pfe, mem + (victim_pfn * PAGE_SIZE));
            victim_pfe->dirty = 0; 
        }
        victim_pfe->valid = 0;
...
pfn_t select_victim_frame() {
    ...
    if (replacement == RANDOM) {
        ...
        for (pfn_t i = 0; i < NUM_FRAMES; i++) {
            pfn_t curr = (last_evicted + i + 1) % num_entries;
        ...
        for (pfn_t i = 0; i < NUM_FRAMES * 2; i++) {
            pfn_t curr_ptr = (last_evicted + i + 1) % num_entries;
            if (!frame_table[curr_ptr].referenced && !frame_table[curr_ptr].protected) {
                last_evicted = curr_ptr;
                return curr_ptr;
            } else if (!frame_table[curr_ptr].protected) {
                frame_table[curr_ptr].referenced = 0;
            }
        }
    }
