...
void proc_init(pcb_t *proc) {
    ...
    memset((mem + (newpfn*PAGE_SIZE)), 0, PAGE_SIZE);
    proc->saved_ptbr = newpfn;
    frame_table[newpfn].protected = 1;
    frame_table[newpfn].mapped = 1;
    frame_table[newpfn].process = proc;
}
...
void proc_cleanup(pcb_t *proc) {
    ...
    pte_t *base_pte = (pte_t*) (mem + proc->saved_ptbr * PAGE_SIZE);
    pte_t *curr_pte;
