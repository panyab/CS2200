...
"""lc2199a.py: A definition of the RAMA-2200 architecture."""
...
SKP_MODE_BITS = 1
SKP_UNUSED_SIZE = 19
assert(OPCODE_WIDTH + SKP_MODE_BITS + REGISTER_WIDTH *
       2 + SKP_UNUSED_SIZE == BIT_WIDTH)  # Sanity check
...
class shift(Instruction):
    ...
    def parse_operands(cls, operands, pc, instruction):
        ...
        if instruction == 'sll':
            ...
            raise RuntimeError("'shift' instruction could not be assembled")
        ...
        result_list.append(parse_register(match.group('RY')))
        # Insert unused bits
        result_list.append('0' * 14)
        result_list.append(zero_extend(mode, 2))
        result_list.append(parse_register(match.group('RZ')))
        return ''.join(result_list)
...
class br(Instruction):
    ...
    def parse_operands(cls, operands, pc, instruction):
        ...
        # Relative PC Address.
        result_list.append('0' * 2 * RELATIVE_UNUSED_BITS)
...
class skp(Instruction):
    ...
    def parse_operands(cls, operands, pc, instruction):
        ...
        if instruction == 'skpgt':
            mode = bin(1)
        elif instruction == 'skpeq':
            mode = bin(0)
        else:
